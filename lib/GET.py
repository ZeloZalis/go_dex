...
def Create_table():
    ...
    try:
        cursor = con.cursor()
        cursor.execute("""CREATE TABLE IF NOT EXISTS pokemon_test (pokeid SERIAL PRIMARY KEY, dexnum INTEGER UNIQUE NOT NULL, name TEXT NOT NULL, type1 TEXT NOT NULL, type2 TEXT, atk INTEGER NOT NULL, hp INTEGER NOT NULL, def INTEGER NOT NULL)""")
        print("Tabla creada.")
        cursor.execute("""CREATE SEQUENCE id_increment start 1 increment 1""")
        print("Secuencia creada.")
        query2 = ("null", 1, "charizard", "fire", "flying", 111, 222, 333)
        # query = """nextval ('id_increment'), 1, 'charizard', 'fire', 'flying', 123, 321, 324"""
        insert_query = """INSERT INTO pokemon_test (pokeid, dexnum, name, type1, type2, atk, hp, def) VALUES (%s, %s, %s, %s, %s, %s, %s, %s,)"""
